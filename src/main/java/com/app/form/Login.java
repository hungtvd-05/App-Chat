                            ...
                            public void call(Object... os) {
                                if (os.length > 0) {
                                    ...
                                    if (action) {
                                        ...
                                        Model_Save_Key msk = KeyUtil.getInstance().getKeyFromDB(Service.getInstance().getUserAccount().getUserId());

                                        Model_Key key;

                                        if (msk == null) {
                                            key = KeyUtil.getInstance().createdKey();
                                            msk = KeyUtil.getInstance().saveKey(Service.getInstance().getUserAccount().getUserId(), key);
                                        } else {
                                            key = KeyUtil.getInstance().decodeKey(msk);
                                        }
                                        
                                        Session.getInstance().setKey(key);
                                        Service.getInstance().getClient().emit("change_key", msk.toJsonObject());
